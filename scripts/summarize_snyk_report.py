...
def format_vuln(vuln):
    ...
    if vuln["fixed_in"]:
        lines.append(f"FixedIn: {', '.join(vuln['fixed_in'])}")
    lines.append(f"Upgradable: {vuln['is_upgradable']}")
    lines.append(f"Patchable: {vuln['is_patchable']}")
    for url in vuln["urls"][:3]:
        lines.append(f"Reference: {url}")
    if vuln["description"]:
        lines.append(f"Description: {vuln['description'][:1000].replace('\n', ' ')}")
...
def generate_report(all_vulns):
    ...
    for vuln in all_vulns:
        report_lines.extend(format_vuln(vuln))
    return "\n".join(report_lines)
...
def main():
    ...
    try:
        ...
        with open("scripts/snyk-vulnerabilities.txt", "w", encoding="utf-8") as f:
            f.write(generate_report(all_vulns))
